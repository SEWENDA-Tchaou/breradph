...
function BackgroundBanner() {
  ...
  const getContents = async() => {
    const response = await axios.get("http://localhost:3000/background");
  ...
  const saveForm = async() => {
    ...
    try {
      await axios.post("http://localhost:3000/background", formData, {
        headers:{
  ...
  const deleteContent = async(contentId) => {
    try {
      await axios.delete(`http://localhost:3000/background/${contentId}`);
      getContents();
    }catch(error){
      console.log(error);
    }
